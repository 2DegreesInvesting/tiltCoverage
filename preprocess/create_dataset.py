...
def create_ecoinvent(df_validation, data_dir, labelled):
    ...
    if labelled:
        y = df_validation["activity_uuid_product_uuid"].tolist()
        dataset["y"] = y
...
def create_ecoinvent_labelled_data_orbis(df, data_dir: str, save_dir: str, name: str):

    val_dataset = create_ecoinvent(df, data_dir=data_dir, labelled=True)
    utils.write_json(f"{save_dir}/orbis_ecoinvent_{name}_dataset.json", val_dataset)


def create_ecoinvent_test_data_orbis(df_training, data_dir: str, save_dir: str):

    val_dataset = create_ecoinvent(df_training, data_dir=data_dir, labelled=False)
    utils.write_json(f"{save_dir}/orbis_ecoinvent_training_dataset.json", val_dataset)
